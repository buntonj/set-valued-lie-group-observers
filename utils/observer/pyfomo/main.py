...
def fourier_motzkin_eliminate_single(var_index,A,b,C=None,d=None,atol=10**-8,verbose=False):
    """
    ...
    if type(C)==type(np.array([1])):
        A=np.vstack((A,C,-C))
        b=np.vstack((b,d,-d))
        return fourier_motzkin_eliminate_single(var_index,A,b,None,None,atol=atol,verbose=verbose)
    else:
        ...
        return canonical_polytope(A_new,b_new,None,atol=atol,verbose=verbose)
