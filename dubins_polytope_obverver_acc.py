### SAME AS OTHER DUBINS POLYTOPE OBSERVER EXAMPLE
# This one uses acceleration inputs to the system
# u = (u_ang,u_trans) where u_ang is angular acceleration and u_trans is translational acceleration
...
from utils.shared.se2_x_R2_utils import *
...
if apx_se2 :
    exp = apx_se2_x_R2_exponential
    log = apx_se2_x_R2_logarithm
else :
    exp = se2_x_R2_exponential
    log = se2_x_R2_logarithm


x0 = 2.0*(rng.random((5,))-0.5) # initial position and heading, random in [-1,1]^3
...
num_steps = 10
u = np.array([1.0,1.5]) # accelerate in a circle!
...
xhat0 = 2.0*(rng.random((model.n_state,))-0.5)
reference = dubins_car(xhat0,output,acc_inputs=True) # initializing the reference model
...
estimate_center_group = (np.zeros((3,3,num_steps)),np.zeros((2,num_steps))) # group element representation of estimate center
set_radius = np.zeros((num_steps,))

error_LR = 'R' # left or right invariant error (this code is initially built for R)

if error_LR == 'L':
    error = left_invariant_error_se2_x_R2 # function that computes error element in SE(2)
    error_to_state = invert_left_invariant_error_se2_x_R2
elif error_LR == 'R':
    error = right_invariant_error_se2_x_R2
    error_to_state = invert_right_invariant_error_se2_x_R2
else :
    raise ValueError('Invalid choice of left/right invariant error.')

# setting up polytope observer in lie algebra of group
set_n_state = 5 # we lift the error dynamics into R^3 so that the output map is linear
...
set_sys_C = np.eye(N=set_n_state-2,M=set_n_state)
#set_sys_C[3:,3:] = 0
...
estimate_center_group[0][:,:,0],estimate_center_group[1][:,0] = error_to_state(exp(error_center),reference.state_group_rep())
estimate_center[:,0] = log((estimate_center_group[0][:,:,0],estimate_center_group[1][:,0]))
...
ybar = reference.state_group_rep()[0] @ y[:,:,0]# np.vstack((y[:,:,0],np.ones((1,num_landmarks))))
#print(reference.left_inverse(ybar))
vectorized_output = special_euclidean_logarithm(reference.left_inverse(ybar))
#print('xibar:',*xibar)
...
for step in range(1,num_steps):
    ...
    ybar = reference.state_group_rep()[0] @ y[:,:,step]
    vectorized_output = special_euclidean_logarithm(reference.left_inverse(ybar))#np.reshape(ybar[0:2,:], (2*num_landmarks,), order='F')
    ...
    error_center = observer.chebyshev_center
    estimate_center_group[0][:,:,step],estimate_center_group[1][:,step] = error_to_state(exp(error_center),reference.state_group_rep()) # group element representing center of estimate set
    estimate_center[:,step] = se2_x_R2_state((estimate_center_group[0][:,:,step],estimate_center_group[1][:,step]))#log(estimate_center_group[:,:,step]) # lie algebra element in R^3 representing center of estimate set
...
sampled_states = np.empty((set_n_state,num_samples))

for i in range(num_samples):
    sampled_states[:,i] = se2_x_R2_state(error_to_state(exp(sampled_polytope[:,i]),reference.state_group_rep((xref[:3,-1],xref[3:,-1]))))
...
def sample_states(i,num_samples): # randomly generates num_samples feasible states from error polytope at timestep i
    ...
    sampled_states = np.empty((5,num_samples))
    for j in range(num_samples):
        sampled_states[:,j] = se2_x_R2_state(error_to_state(exp(sampled_polytope[:,j]),reference.state_group_rep((xref[:3,i],xref[3:,i]))))
